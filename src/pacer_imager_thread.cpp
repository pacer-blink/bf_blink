...
: m_szAntennaLocationFile(szAntennaLocationFile), m_pImagingQueue(pImagingQueue)
{
}
...
static void* RunImagingThread( void* pPtr )
{
   ...
   if( strlen(pImagingParameters->m_szAntennaLocationFile.c_str()) > 0 ){
      pacer_imager.m_ImagerParameters.SetParameters( pImagingParameters->m_szAntennaLocationFile.c_str() );
   }
